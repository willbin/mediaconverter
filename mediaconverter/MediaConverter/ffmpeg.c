...
// Jackie Ou added
#define ARGC_VALUES 8

// Jackie Ou added
static int _argc = ARGC_VALUES;

// Jackie Ou added
static char* _argv[ARGC_VALUES] = {0};

// Jackie Ou added
static void free_params_mem() {
...
// Jackie Ou added
static void malloc_params_mem() {
    //free_params_mem();
    for (int i = 0; i < _argc; ++i) {
        _argv[i] = malloc(sizeof(char)*256);
        memset(_argv[i], 0, 256);
    }
}

// Jackie Ou added
static int transcode_impl(int argc, char **argv);
...
// Jackie Ou added
int transcoding(char* input_filename, char* output_filename, void* callback)
{
    _exit_callback = (exit_callback)callback;
    
    malloc_params_mem();
    
    memcpy(_argv[0], "ffmpeg", strlen("ffmpeg"));
    memcpy(_argv[1], "-i", strlen("-i"));
    memcpy(_argv[2], input_filename, strlen(input_filename));
    memcpy(_argv[3], output_filename, strlen(output_filename));
    memcpy(_argv[4], "-codec:v", strlen("-codec:v"));
    memcpy(_argv[5], "mpeg4", strlen("mpeg4"));
    memcpy(_argv[6], "-acodec", strlen("-acodec"));
    memcpy(_argv[7], "copy", strlen("copy"));
    
    return transcode_impl(_argc, _argv);
}

// Jackie Ou modified
int transcode_impl(int argc, char **argv)
{
    int i, ret;
    int64_t ti;
